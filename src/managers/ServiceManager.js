...
async getServicesByCategory(category) {
  ...
  if (filteredServices.length === 0) {
    throw new Error(`No se encontraron servicios que coincidan con la categoria: ${category}`);
  }
...
async getServicesByAvailable (availableStatus) {
  ...
  const filteredAvailable = services.filter(s => 
    s.available === availableStatus);
