...
servicesRouter.get("/", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Todos los servicios", data });
  } catch (error) {
  ...
  try {
    ...
    if (!name || !price) {
      return res
        ...
        .json({
          status: "error",
          message: "Faltan campos obligatorios (name, price)",
        });
    }
...
servicesRouter.delete("/:id", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Dato eliminado con éxito", data });
  } catch (error) {
    return res
      .status(404)
      .json({
        success: false,
        message: error.message || "El dato no pudo ser eliminado",
      });
  }
});
